...
#define NUM_OF_ENTRIES 90000
#define NUM_OF_REGIONS 4

char *ObjectNames[NUM_OF_ENTRIES];
FILE *object_mappings = NULL;

int OutOfOrder  = 0;
int Endian      = NO_ENDIAN;
int PrintLog    = TRUE;
int PrintOnlyTs = TRUE;
...
uint32_t ServersPerRegion   [NUM_OF_REGIONS];
uint32_t RequestsPerRegion  [NUM_OF_REGIONS];
double   TotalBytesPerRegion[NUM_OF_REGIONS];
uint32_t StartTimePerRegion [NUM_OF_REGIONS];
uint32_t LastTimePerRegion  [NUM_OF_REGIONS];
double   ReqInterArrivalTimePerRegion[NUM_OF_REGIONS];

/**************************************************************************/
/* DEFINITIONS                                                            */
/**************************************************************************/
char ProtocolNames[NUM_OF_PROTOCOLS][20] = {
    "HTTP/0.9",
    "HTTP/1.0",
    "HTTP/1.1",
    "HTTP/X.X"
...
char MethodNames[NUM_OF_METHODS][20] = {
    "GET",
    "HEAD",
    "POST",
    "PUT",
...
typedef enum {
    SANTA_CLARA_CALIFORNIA,
    PLANO_TEXAS,
    HERNDON_VIRGINIA,
    PARIS_FRANCE
} REGION;

char RegionNames[NUM_OF_REGIONS][22] = {
    "SANTA_CLARA_CALIFORNIA",
    "PLANO_TEXAS",
    "HERNDON_VIRGINIA",
    "PARIS_FRANCE"
...
// char RequestTypesPerRegion[NUM_OF_REGIONS][7];
uint32_t RequestTypesPerRegion[NUM_OF_REGIONS][NUM_OF_METHODS];

/**************************************************************************/
/* READ LOG                                                               */
/**************************************************************************/
void ReadLog() {
    struct request BER, LER, *R = NULL;
    int value = 0;
    uint32_t count = 0;
    int raw, part1,part2;
    int region = 0, server = 0;
    int http = 0, status = 0;
    int method = 0, type = 0;
    time_t ts;
    ...
    while((!feof(stdin)) && (1)) {
        ...
        if ( (region >= 0) && (region < 4) ) {
            RequestsPerRegion[region]++;
        } else {
            fprintf(stderr, "Bad region number: %u \n", region);
            exit(-1);
        }

        if ( server < 0 ) {
            fprintf(stderr, "Bad server number: %u\n", server);
            exit(-1);
        } else {
            if ( ServersPerRegion[region] < server ) {
                ServersPerRegion[region] = server;
            }
        }

        /* This is the first timestamp for this region */
        if (StartTimePerRegion[region] == -1) {
            StartTimePerRegion[region] = R->timestamp;
            ReqInterArrivalTimePerRegion[region] += (double) (R->timestamp - StartTimePerRegion[region]);
            LastTimePerRegion [region] = R->timestamp;
        } else {
            ReqInterArrivalTimePerRegion[region] += (double) (R->timestamp - LastTimePerRegion[region]);
            LastTimePerRegion [region] = R->timestamp;
        }

        if(R->size != NO_SIZE) {
            TotalBytesPerRegion[region] += R->size;
        }

        /* Update the input rate metric and the last timestamp for this region */
        ...
        RequestTypesPerRegion[region][method]++;
        ...
        if(PrintLog == TRUE) {
            ...
            // Only timestamps in UNIX time and region numbers
            } else {
                fprintf(stdout,"%ld %d\n", ts, region);
            }
        }
...
void PrintResults() {
    ...
    fprintf(stderr, "Mean Transfer Size: %f bytes\n", TotalBytes/(double)TotalRequests);
    ...
    for(int i=0; i<NUM_OF_REGIONS; i++) {
        ...
        fprintf(stderr, "\tNumber of  servers: %u\n",   ServersPerRegion   [i]);
        fprintf(stderr, "\tNumber of requests: %u\n",   RequestsPerRegion  [i]);
        fprintf(stderr, "\tNumber of    bytes: %.0f\n", TotalBytesPerRegion[i]);
        if ( RequestsPerRegion[i] > 0 ) {
            double transSize = TotalBytesPerRegion[i]/(double)RequestsPerRegion[i];
            fprintf(stderr, "\tMean Transfer Size: %f bytes\n", transSize);
        }
        fprintf(stderr, "\t        Start time: %d\n",   StartTimePerRegion [i]);
        fprintf(stderr, "\t       Finish time: %d\n",   LastTimePerRegion  [i]);
        if ( RequestsPerRegion[i] > 0 ) {
            double sec = ReqInterArrivalTimePerRegion[i]/(double)RequestsPerRegion[i];
            fprintf(stderr, "\tAverage request inter-arrival time: %f seconds (%f ms)\n", sec, sec*1000);
        }
        fprintf(stderr, "\n");

        int found = 0;
        for (int j=0; j<NUM_OF_METHODS; j++) {
            if ( RequestTypesPerRegion[i][j] > 0 ) {
                found = 1;
                fprintf(stderr, "\tNumber of %7s requests: %u\n", MethodNames[j], RequestTypesPerRegion[i][j]);
            }
        }
...
void Initialize(int argc, char **argv) {
    ...
    /* initialize region and server information */
    for(i=0; i<NUM_OF_REGIONS; i++) {
        ServersPerRegion   [i] = 0;
        RequestsPerRegion  [i] = 0;
        TotalBytesPerRegion[i] = 0.0;
        StartTimePerRegion [i] = -1;
        LastTimePerRegion  [i] = 0;
        ReqInterArrivalTimePerRegion[i] = 0.0;

        for (int j=0; j<NUM_OF_METHODS; j++) {
            RequestTypesPerRegion[i][j] = 0;
        }
    }
    ...
    while(!feof(object_mappings)) {
        ...
        if((value == 2) && (ID >= 0) && (ID < NUM_OF_ENTRIES)) {
        ...
        } else if((value == 2) && (ID >= NUM_OF_ENTRIES)) {
